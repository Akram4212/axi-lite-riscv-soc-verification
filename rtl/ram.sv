module ram #(
    ...
    // This temporary RAM model uses only the local word-index bits.
    // Byte-alignment and upper address bits are intentionally ignored.
    ...
    logic [ADDR_W-1:0] request_index;
    ...
    always_ff @(posedge CLK or negedge nRST) begin
        if (!nRST) begin
            ...
            if (!busy && (ramREN || ramWEN)) begin
                request_index <= ramaddr[ADDR_W+1:2];
